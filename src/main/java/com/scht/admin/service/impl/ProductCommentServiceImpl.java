...
public class ProductCommentServiceImpl implements ProductCommentService {
    ...
    @Override
    public RetResult list(String productId, int pageNo, int pageSize) {
        RetResult result = null;
        try{
            if(pageNo<1)
                pageNo = 1;
            List<ProductComment> list = this.productCommentDao.list(productId,(pageNo-1)*pageSize,pageSize);
            Integer total = this.productCommentDao.count(productId);
