...
<script type="text/javascript">
  ...
  BUI.use(['common/search','common/page','bui/grid','bui/overlay','bui/form'],function (Search,Page,Grid,Overlay,Form) {
    var  columns = [
              ...
              {title:'权限码',dataIndex:'url',width:150},
              {title:'操作',dataIndex:'',width:200,renderer : function(value,obj){
                var editStr =  Search.createLink({ //链接使用 此方式
                          id : 'edit' + obj.id,
                          title : '编辑',
                          text : '编辑',
                          href : '/permission/beforeEdit?id='+obj.id
                        });
                  var delStr = "<a href=\"javascript:void(0);\" onclick=\"deleteP('"+obj.id+"','1')\">删除</a>";

                return editStr+delStr;
              }}
            ];
            store = Search.createStore('/permission/listData');
            gridCfg = Search.createGridCfg(columns,{
              tbar : {
                items : [
                  {text : '<i class="icon-plus"></i>新建',btnCls : 'button button-small',handler:function(){
                      top.topManager.openPage({id:"role_edit",href:"/permission/beforeEdit",title:"新建"});
                  }}
                ]
              },
