  ...
  <form id="J_Form" class="form-horizontal" action="/admin/save">
    ...
    <div class="row">
      <div class="control-group span20">
        ...
        <div class="controls">
          <input name="password" type="password" data-rules="{required:true,maxlength:50}" value="${admin.password}" class="input-normal control-text"
                  <c:if test="${admin.password!=null}">readonly</c:if>>
        </div>
      </div>
    </div>
    <div class="row">
        ...
        <div class="controls">
          <select  class="input-normal" name="roleId" id="roleId">
            <c:forEach items="${list}" var="e">
              <option value="${e.id}" <c:if test="${e.id eq data.roleId}">selected</c:if>>${e.roleName}</option>
            </c:forEach>
          </select>
