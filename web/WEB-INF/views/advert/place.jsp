...
<script type="text/javascript">
  ...
  BUI.use(['common/search','common/page','bui/grid','bui/overlay','bui/form'],function (Search,Page,Grid,Overlay,Form) {
    var  columns = [
      {title:'广告位名称',dataIndex:'title',width:'auto'},
      {title:'调用代码',dataIndex:'code',width:150},
      {title:'宽度X高度',dataIndex:'', width:150,renderer : function(value,obj){
      ...
      {title:'操作',dataIndex:'aaa',width:100,renderer : function(value,obj){
        var editStr =  Search.createLink({ //链接使用 此方式
          ...
          title : '编辑',
          text : '编辑',
          href : '/advertPlace/find?id='+obj.id
        });
        var delStr =  "&nbsp;&nbsp;<a href=\"javascript:void(0);\" onclick=\"del('"+obj.id+"')\">删除</a>&nbsp;&nbsp;";
        return editStr+delStr;
      }}
    ];
    store = Search.createStore('/advertPlace/listData');
    gridCfg = Search.createGridCfg(columns,{
      tbar : {
        items : [
          {text : '<i class="icon-plus"></i>新建',btnCls : 'button button-small',handler:function(){
            top.topManager.openPage({id:"advert_place_add",href:"/advertPlace/add",title:"新增广告位"});
          }}
        ]
      },
  ...
  function del(id){
    BUI.Message.Confirm("确定删除该广告位吗？",function(){
      $.ajax({
        url :"/advertPlace/delete?r="+Math.random(),
        type : 'post',
        data : {id:id},
